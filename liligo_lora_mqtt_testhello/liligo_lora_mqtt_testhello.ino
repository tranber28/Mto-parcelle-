...
// Variable pour suivre le dernier envoi de message
...
void loop() {
  ...
  if (packetSize) {
    ...
    // Réinitialiser le dernier temps de message
    //lastMsgTime = millis();
  /*} else {
    // Vérifiez si 10 secondes se sont écoulées depuis le dernier message
    if (millis() - lastMsgTime >= interval) {
      Serial.println("Envoi de Hello World par défaut");
